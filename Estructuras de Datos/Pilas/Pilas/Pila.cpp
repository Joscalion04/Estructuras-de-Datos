...
Pila::~Pila() 
{
    while (!estaVacia()) {
        ...
        Top = Top->getSiguiente();  // CAMBIO LA DIRECCION DEL DE ARRIBA
...
void Pila::push(Persona* persona) //INMEDIATAMENTE O(1) PARA EL MEJOR DE LOS CASOS
{
    ...
    nuevoNodo->setSiguiente(Top); //nuevoNodo->setAbajo(Top)
...
void Pila::pop() //INMEDIATAMENTE O(1) PARA EL MEJOR DE LOS CASOS
{
    ...
    Top = Top->getSiguiente(); // REASIGNA EL TOP AL QUE ESTA ABAJO DEL TOP
